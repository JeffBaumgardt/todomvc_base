...
const Footer = ({ todos, chooseData, filter, clearCompleted }) => {
  ...
  let completedButtonClass =
    todosCompleted.length > 0 ? "clear-complete" : "clear-complete hide";
  ...
  return  (
    <div className="Footer">
      <div className="todos-left">
        {todosLeftToComplete.length}{" "}
        <span className="mobile-hide">{phrasing}</span> left
      </div>
      <div className="nav-buttons">
        ...
        <div
          className={`active-elements ${active}`}
          onClick={() => chooseData("Active")}
        >
          Active
        </div>
        <div
          className={`completed ${completed}`}
          onClick={() => chooseData("Completed")}
        >
          Completed
        </div>
      </div>
